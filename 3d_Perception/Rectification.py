...
class MonoRectification:
    ...
    def computeCorrectionMaps(self, alpha = 1.0, xRatio = 1, yRatio = 1):
        ...
        newcameramtx, roi = cv.getOptimalNewCameraMatrix(self.cameraMatrix, self.distCoeffs, self.imageSize, alpha, newImageSize)
        # Implement initUndistortRectifyMap here
        self.mapx, self.mapy = cv.initUndistortRectifyMap(self.cameraMatrix, self.distCoeffs, None, newcameramtx, newImageSize, cv.CV_32FC1)
        
    def rectify(self, frame):
        dst = cv.remap(frame, self.mapx, self.mapy, cv.INTER_LINEAR)
        return dst # Implement remap here
...
class StereoRectification:
    ...
    def computeCorrectionMaps(self, alpha = 1.0, ratio = 1):
        ...
        RLeft, RRight, PLeft, PRight, Q, validPixROI1, validPixROI2 = cv.stereoRectify(self.cameraMatrixLeft, self.distCoeffsLeft, self.cameraMatrixRight, self.distCoeffsRight, self.imageSize, self.R, self.T, alpha=alpha, newImageSize=newImageSize)
