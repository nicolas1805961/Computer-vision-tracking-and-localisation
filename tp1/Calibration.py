...
class CalibrationBase:
    ...
    def detect(self, frame, refine = False):    
        ...
        if self.patternType == 'chessboard':
            ...
            patternFound, corners = cv.findChessboardCorners(frame, (self.cols, self.rows), None)
...
class MonoCalibration(CalibrationBase):      
    ...
    def calibrate(self, framesPath = 'results/acquired'):
        ...
        ret, self.cameraMatrix, self.distCoeffs, self.rvecs, self.tvecs, stdDeviationsIntrinsics, stdDeviationsExtrinsics, self.perViewErrors = cv.calibrateCameraExtended(objectPoints, imgPoints, imageSize, None, None)
    ...
    def visualizeBoards(self, cameraWidth = 10.0, cameraHeight = 5.0):       
...
class StereoCalibration(CalibrationBase):        
    def calibrate(self, framesPathLeft = 'data/stereo/calibration/left', framesPathRight  = 'data/stereo/calibration/right'):        
        ...
        retval, self.cameraMatrixLeft, self.distCoeffsLeft, self.cameraMatrixRight, self.distCoeffsRight, self.R, self.T, E, F, self.perViewErrors = cv.stereoCalibrateExtended(objectPointsRight, imgPointsLeft, imgPointsRight, None, None, None, None, imageSizeRight, None, None, flags=cv.CALIB_FIX_K3)
